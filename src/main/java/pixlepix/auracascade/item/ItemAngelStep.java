...
public class ItemAngelStep extends Item implements IBauble, ITTinkererItem {
    ...
    public void onWornTick(ItemStack itemStack, EntityLivingBase player) {
        if (player.isCollidedHorizontally) {
            player.stepHeight += .3;
            AuraCascade.proxy.networkWrapper.sendToAllAround(new PacketBurst(8, player.posX, player.posY - 1.5, player.posZ), new NetworkRegistry.TargetPoint(player.world.provider.getDimension(), player.posX, player.posY, player.posZ, 32));
