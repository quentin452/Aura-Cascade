...
public class CraftingCenterTile extends TileEntity {
    ...
    public boolean pedestalsConnected() {
        for (EnumFacing direction : pedestalRelativeLocations) {
            if (!(world.getTileEntity(getPos().offset(direction)) instanceof AuraTilePedestal)) {
                return false;
            }
        }
        return true;
    ...
    public PylonRecipe getRecipe() {
        ...
        for (EnumFacing direction : pedestalRelativeLocations) {
            AuraTilePedestal pedestal = (AuraTilePedestal) world.getTileEntity(getPos().offset(direction));
    ...
    public void checkRecipeComplete() {
        ...
        if (valid) {
            ...
            EntityItem entityDrop = new EntityItem(world, getPos().getX() + .5, getPos().getY() + 2, getPos().getZ() + .5, loot);
            world.spawnEntity(entityDrop);
            AuraCascade.proxy.networkWrapper.sendToAllAround(new PacketBurst(3, getPos().getX() + .5, getPos().getY() + 2, getPos().getZ() + .5), new NetworkRegistry.TargetPoint(world.provider.getDimension(), getPos().getX(),getPos().getY(), getPos().getZ(), 32));
    ...
    public void burst(BlockPos origin, BlockPos target, String particle) {
        ...
        AuraCascade.proxy.networkWrapper.sendToAllAround(new PacketBurst(origin, target, particle, 1, 1, 1), new NetworkRegistry.TargetPoint(world.provider.getDimension(), getPos().getX(),getPos().getY(), getPos().getZ(), 32));
